...
class Config:
    ...
    # EXPERIMENT_02_SNR_AWARE ADDITION: per-target-SNR transparency/identity
    # regulariser. Pulls `enhanced` toward the NOISY input (not clean),
    # weighted by how clean the input already is. Zero for -5/0/+5 dB, so
    # training in that regime is UNCHANGED from the corrected baseline -
    # this is the entire "controlled experiment" property: if low-SNR
    # results move at all, something else broke.
    #
    # Calibration (measured on real +10/+20 dB test files - see
    # EXPERIMENT_02_DESIGN.md): mean|noisy-clean| = 0.0055 at +10 dB and
    # 0.0034 at +20 dB - the noisy input is already very close to clean at
    # high SNR, so "transparent" is a small, easy-to-hit target, not an
    # arbitrary one. Weights are set to roughly DOUBLE at each step up
    # (10->15->20) to encode "increasingly conservative", and 40 at +20 dB
    # is deliberately kept below waveform_loss_weight (50) so this term
    # reinforces, rather than overrides, the existing denoising objective -
    # even a file labelled "+20 dB" can have brief louder noise moments
    # worth suppressing, and this term is not meant to prevent that.
    transparency_loss_weights: Optional[Dict[float, float]] = None
    use_transparency_loss: bool = True
    # EXPERIMENT_02_SNR_AWARE ADDITION: keeps this run's checkpoint/history/
    # results completely separate from the existing corrected baseline.
    # output_dir is computed from models_root/experiment_name in
    # __post_init__ UNLESS explicitly overridden.
    experiment_name: str = "Experiment_02_SNR_Aware"
    models_root: Path = Path(r"C:\SIH\models")
    random_seed: int = 42
    patience: int = 15
    ...
    def __post_init__(self) -> None:
        ...
        if self.transparency_loss_weights is None:
            self.transparency_loss_weights = {
                -5.0: 0.0, 0.0: 0.0, 5.0: 0.0, 10.0: 5.0, 15.0: 20.0, 20.0: 40.0,
            }
        if self.output_dir is None:
            self.output_dir = self.models_root / self.experiment_name
...
def si_snr_loss(estimate: Tensor, target: Tensor) -> Tensor:
    ...
    is clamped to be non-negative. A negative alpha means the model's
    output is correlated with -clean rather than clean - a polarity
    inversion - which the unclamped formula scored identically to a
    correctly-signed reconstruction of the same magnitude, because the
    ratio depends on alpha^2, not alpha. Do NOT remove this clamp.
    """
    estimate = estimate - estimate.mean(dim=-1, keepdim=True)
    target = target - target.mean(dim=-1, keepdim=True)
    dot = (estimate * target).sum(-1, keepdim=True)
    target_energy = target.pow(2).sum(-1, keepdim=True) + 1e-8
    alpha = torch.clamp(dot / target_energy, min=0.0)  # sign-inversion fix
...
def transparency_loss(
    estimate: Tensor,
    noisy: Tensor,
    target_snr_values: Sequence[float],
    weights_by_snr: Dict[float, float],
) -> Tensor:
    """EXPERIMENT_02_SNR_AWARE: SNR-aware conservativeness regulariser.

    Unlike every other loss in this file, this one is measured against the
    NOISY input, not clean - it directly penalises *changing* the signal,
    weighted per-sample by how clean that signal already is (target_snr_db).
    At -5/0/+5 dB the weight is 0 by construction, so this term contributes
    exactly nothing there and gradients in that regime are IDENTICAL to the
    corrected baseline. At +10/+15/+20 dB it grows, discouraging the model
    from applying its denoising nonlinearity to signals that need little or
    no denoising - the specific failure mode diagnosed in
    EXPERIMENT_02_DESIGN.md (SI-SNR/spectral losses don't diminish their
    pull just because there's little noise to remove; this term is the one
    mechanism in the objective that does).
    """
    diff = (estimate - noisy).abs().mean(dim=-1)  # per-sample L1, shape [B]
    weights = diff.new_tensor(
        [weights_by_snr.get(round(float(s), 1), 0.0) for s in target_snr_values]
    )
...
def sanity_check(config: Config, device: torch.device, loader: DataLoader) -> None:
    ...
    passed: List[str] = []
    try:
        noisy, clean, metadata = next(iter(loader))
        ...
        print(f"  Total Loss (pre-transparency) : {total.item():.6f}")
        passed.append("Loss calculation")

        # EXPERIMENT_02_SNR_AWARE (Section: smoke test) - verify the new
        # transparency term computes cleanly and behaves as designed: it
        # must be exactly 0.0 for every -5/0/+5 dB sample in this batch.
        stage = "transparency loss (Experiment 2)"
        batch_snrs = [float(s) for s in metadata["target_snr_db"]]
        transparency = transparency_loss(
            enhanced, noisy, batch_snrs, config.transparency_loss_weights
        )
        total_with_transparency = total + transparency
        print(f"  Batch target SNRs (dB) : {batch_snrs}")
        print(f"  Transparency Loss      : {transparency.item():.6f}")
        low_snr_in_batch = [s for s in batch_snrs if s <= 5.0]
        if low_snr_in_batch and not config.use_transparency_loss:
            pass  # disabled entirely - nothing to check
        elif low_snr_in_batch and all(
            config.transparency_loss_weights.get(round(s, 1), 0.0) != 0.0
            for s in low_snr_in_batch
        ):
            raise RuntimeError(
                "transparency_loss_weights has a non-zero weight at -5/0/+5 dB - "
                "this breaks the 'controlled experiment' guarantee."
            )
        print(f"  Total Loss (with transparency) : {total_with_transparency.item():.6f}")
        passed.append("Transparency loss (Experiment 2)")
        ...
        stage = "NaN/Inf check"
        if not finite((noisy, clean, noisy_spec.real, output, enhanced, si, spectral,
                       amplitude, waveform, transparency, total_with_transparency)):
        ...
        total_with_transparency.backward()
...
def run_epoch(
    ...
    totals: Dict[str, List[float]] = {
        ...
        "waveform_loss": [], "transparency_loss": [], "total_loss": [],
    ...
    for noisy, clean, metadata in loader:
        ...
        # EXPERIMENT_02_SNR_AWARE: computed and added to `total` in BOTH
        # train and eval mode (not just when optimizer is not None), so
        # val_total_loss - the checkpoint-selection criterion - reflects the
        # complete objective, not a subset of it. losses() itself is left
        # completely untouched (see EXPERIMENT_02_DESIGN.md) - this term is
        # added on top, exactly the "single, isolated change" the brief
        # asked for.
        if config.use_transparency_loss:
            target_snr_values = [float(s) for s in metadata["target_snr_db"]]
            transparency = transparency_loss(
                enhanced, noisy, target_snr_values, config.transparency_loss_weights
            )
        else:
            transparency = torch.zeros((), device=device)
        ...
        totals["transparency_loss"].append(float(transparency.detach().cpu()))
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        record: Dict[str, object] = {
            ...
            "train_transparency_loss": train_metrics["transparency_loss"],
            ...
            "val_transparency_loss": val_metrics["transparency_loss"],
        ...
        print(f"    Transparency Loss  : {train_metrics['transparency_loss']:.6f}"
              "  (0 at -5/0/+5 dB by design)")
        ...
        print(f"    Transparency Loss  : {val_metrics['transparency_loss']:.6f}"
              "  (0 at -5/0/+5 dB by design)")
        ...
        results.append(
            {
                ...
                # EXPERIMENT_02_SNR_AWARE: numeric clipping percentage (fraction
                # of samples in this file with |enhanced|>1.0), so it can be
                # averaged in test_set_metrics/snr_summary - 'clipped_for_wav'
                # is boolean and can't be meaningfully averaged.
                "clipping_pct": float((np.abs(enhanced_np) > 1.0).mean() * 100.0),
...
def save_model_report(
    ...
    report = {
        ...
        "transparency_loss_weights": config.transparency_loss_weights,
        "use_transparency_loss": config.use_transparency_loss,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="EXPERIMENT_02_SNR_AWARE: output subfolder name under models_root "
             "(e.g. 'Experiment_02_SNR_Aware'). Keeps this run's checkpoint/"
             "history/results completely separate from other experiments - "
             "never overwrites another experiment's output_dir. Default: "
             "'Baseline_Corrected' (config default).",
    )
    parser.add_argument(
        "--no-transparency-loss", action="store_true",
        help="EXPERIMENT_02_SNR_AWARE: disable the transparency loss term "
             "entirely (transparency contributes exactly 0.0 at every SNR), "
             "reproducing the corrected-baseline objective exactly for "
             "ablation/comparison purposes.",
    )
    ...
    if args.experiment_name is not None:
        ...
        config.output_dir = config.models_root / config.experiment_name
    if args.no_transparency_loss:
        config.use_transparency_loss = False

    print(f"Experiment name    : {config.experiment_name}")
    print(f"Output directory   : {config.output_dir}")
    print(f"Transparency loss  : {'ENABLED' if config.use_transparency_loss else 'disabled'}"
          f"  weights={config.transparency_loss_weights}")
